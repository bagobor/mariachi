...
std::list<unsigned int> AStarPathFinder::findPath(unsigned int startNodeId, unsigned int endNodeId, std::map<unsigned int, Coordinate3d_t *> nodesMap, std::map<unsigned int, std::vector<unsigned int>> neighboursMap) {
    // the found path
    std::list<unsigned int> path;

    // the unexplored nodes ordered by lowest cost
    std::priority_queue<std::pair<unsigned int, float>, std::vector<std::pair<unsigned int, float>>, AStarPathFinder> openSetQueue;
...
bool AStarPathFinder::operator()(std::pair<unsigned int, float> &firstPair, std::pair<unsigned int, float> &secondPair) {
    return firstPair.second > secondPair.second;
}

float AStarPathFinder::calculateDistance(Coordinate3d_t *firstCoordinates, Coordinate3d_t *secondCoordinates) {
